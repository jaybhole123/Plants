...
const SaudaScale = () => {
  ...
  const handleSaveInline = () => {
    ...
    const quantity = Number(editForm.saudaQuantity) || 0
    const prv = Number(editForm.prvPending) || 0
    const dispatch = Number(editForm.qtyDisptch) || 0
    const balPending = (quantity + prv) - dispatch
    ...
    showToast('Sale entry updated successfully.')
    setEditingId(null)
  }
  ...
  const renderForm = () => (
    <div className="bg-white border border-slate-100 rounded p-6 sm:p-8 mb-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        ...
        <div>
          <label className="text-[11px] uppercase tracking-wider text-slate-500 font-semibold mb-1.5 block">Sauda Quantity</label>
          <input type="number" step="any" min="0" value={form.saudaQuantity} onChange={(e) => setForm(prev => ({ ...prev, saudaQuantity: e.target.value }))} className="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-400 rounded-md text-slate-800 text-xs focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200" placeholder="1000" />
        </div>
        <div>
          <label className="text-[11px] uppercase tracking-wider text-slate-500 font-semibold mb-1.5 block">Rate Amt.</label>
          <input type="number" step="any" min="0" value={form.rateAmt} onChange={(e) => setForm(prev => ({ ...prev, rateAmt: e.target.value }))} className="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-400 rounded-md text-slate-800 text-xs focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200" placeholder="8500" />
        </div>
        ...
        <div>
          <label className="text-[11px] uppercase tracking-wider text-slate-500 font-semibold mb-1.5 block">Qty. Dispatch</label>
          <input type="number" step="any" min="0" value={form.qtyDispatch} onChange={(e) => setForm(prev => ({ ...prev, qtyDispatch: e.target.value }))} className="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-400 rounded-md text-slate-800 text-xs focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200" placeholder="0" />
        </div>
        ...
        <div>
          ...
          <input type="text" readOnly value={formatNumber(calculatedBalance)} className="w-full px-4 py-2.5 bg-emerald-50/50 border border-emerald-200 rounded-md text-emerald-700 text-xs font-semibold cursor-not-allowed" />
        </div>
              ...
              <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
                {editingId === item.id ? (
                  <>
                    ...
                    <td className="border border-slate-400 px-2 py-1 text-center"><input type="number" value={editForm.saudaQuantity} onChange={e => setEditForm({...editForm, saudaQuantity: e.target.value})} className="w-20 border rounded px-1 py-0.5 text-xs focus:ring-1 focus:ring-blue-500 outline-none" placeholder="QTY" /></td>
                    ...
                    <td className="border border-slate-400 px-2 py-1 text-center font-bold text-slate-800 bg-slate-50">{formatNumber(((Number(editForm.saudaQuantity)||0) + (Number(editForm.prvPending)||0)) - (Number(editForm.qtyDisptch)||0))}</td>
  ...
  return (
    <div className="space-y-6 pb-10 px-2 sm:px-4 max-w-[1400px] mx-auto text-slate-800">
      ...
      <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded p-6 sm:p-8 mb-8 shadow-lg relative overflow-hidden flex flex-col sm:flex-row items-center justify-between text-white">
        ...
        <div className="z-10 text-center sm:text-left mb-4 sm:mb-0">
          <p className="text-blue-100 text-xs font-semibold uppercase tracking-wider mb-1">Module</p>
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight">Sauda Sale</h1>
        </div>
